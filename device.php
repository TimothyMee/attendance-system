...
<?php
		...
		$device = $queriesObject->getDevice();
...
<?php
		...
		if ($_GET['sn'] == '' || !isset($_GET['sn']) || empty($_GET['sn'])) {
			...
			$sn = $queriesObject->deviceCheckSn($_GET['sn']);
		...
		if (count($res) > 1) {
			...
			$result = $queriesObject->insertNewDevice($data);
		...
		$data = [
			...
			$result1 = $queriesObject->deleteNewDevice($data);
